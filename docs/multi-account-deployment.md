...
sed -i "s/1.1.1.1/47.101.70.166/g" ./patch/patch.json
...
```sh
TOKEN=$(curl -s http://47.101.70.166:4000/auth/login \
  -H 'content-type: application/json' \
  -d '{"username":"ADMIN_USERNAME","password":"ADMIN_PASSWORD"}' \
  | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')

curl -s http://47.101.70.166:4000/auth/invites \
