...
const restrictNetwork = async function(req, res, next) {
  if (!req.params.nwid) return next();
  if (await multiAccount.canAccessNetwork(req.session.user, req.params.nwid)) {
    return next();
  }
  res.status(404).render('error', { title: 'Network not found', error: 'Network not found' });
};
...
router.use('/network/:nwid', restrict, restrictNetwork);
