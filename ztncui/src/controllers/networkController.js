...
exports.network_list = async function(req, res) {
  ...
  try {
    let networks = await zt.network_list();
    await multiAccount.ensureMigration(
      await usersController.get_users(),
      networks.map(network => network.nwid)
    );
    networks = await multiAccount.filterNetworksForUser(req.session.user, networks);
    networks = await multiAccount.annotateNetworkOwners(networks);
...
exports.network_create_post = async function(req, res) {
  ...
  if (errors) {
    ...
    try {
      ...
      await multiAccount.recordNetworkOwner(network.nwid, req.session.user.name);
...
exports.network_delete_post = async function(req, res) {
  ...
  try {
    ...
    await multiAccount.removeNetworkOwner(req.params.nwid);
