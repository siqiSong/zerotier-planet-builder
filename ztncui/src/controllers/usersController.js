...
exports.users_list = async function(req, res) {
  ...
  try {
    ...
    res.render('users', { title: 'User management', navigate: navigate, message: 'List of controller users', users: users });
  } catch (err) {
    res.render('users', { title: 'User management', navigate: navigate, message: 'Error', users: null, error: 'Error returning list of users: ' + err });
  }
}
...
exports.password_post = async function(req, res) {
  ...
  if (errors) {
    ...
    const hash = await hashPassword(req.body.password1);
